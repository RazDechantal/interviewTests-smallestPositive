...
lies strictly between values A[P] and A[Q], and in addition A[P] ≠ A[Q].
...
the following pairs of indices have adjacent values:
  ...
  (2, 4),   (2, 7),   (3, 4),
  (3, 6),   (4, 5),   (5, 7).
For example, indices 4 and 5 have adjacent values because the values A[4] = 5 and
A[5] = 3 are different, and there is no value in array A that lies strictly between
them. The only such value could be the number 4, which is not present in the array.

Given two indices P and Q, their distance  is defined as abs(P−Q), where abs(X) = X
for X ≥ 0, and abs(X) = −X for X < 0. For example, the distance between indices 4
and 5 is 1 because abs(4 − 5) = (5 − 4) = 1.

Write a function:
int solution(vector<int> &A)
that, given a non-empty array A consisting of N integers, returns the minimum distance
between indices of this array that have adjacent values. The function should return −1
if no adjacent indices exist.
...
- indices 1 and 3 are adjacent because A[1] ≠ A[3] and the array does not contain any 
value that lies strictly between A[1] = 4 and A[3] = 3;
