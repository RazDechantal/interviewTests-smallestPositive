...
int main()
{
    ...
    */
    ...
    { // parseInputs returns false if error was found
        return 0;
    }
...
bool parseInputs(string input, map<char, vector<char>> &adjList, map<char, int> &numParents, char &root)
{
    ...
    for (int i = 0; i < input.length(); i++)
    {
        ...
        { // If input[i] is a node
            index++;
            if (index % 2 == 1)
            { // input[i] is a parent node. Store it for use in next iteration.
            ...
            { // input[i] is a child node. Check for input errors, then add to adjacency list if error checks pass
                ...
                { // Opportunity for time complexity improvements here
                    if (adjList[parent][j] == input[i])
                    { // If there is already such a parent/child pair
                        cout << "E2\n";
                        return false;
                    }
                }
                ...
                { // If parent node already has 2 child
                    cout << "E3\n";
                    return false;
                }
        ...
        { // If input[i] is not a tree node, then perform input format check for consecutive spaces (E1)
            if (input[i] == ' ')
            {
                cout << "E1\n";
                return false;
    ...
    {
        if (kv.second == 0)
        {                    // If node has no parents, it has to be a root
            root = kv.first; // Set root of tree for S-Expression printout later
            numRoots++;
            if (numRoots == 2)
            { // If tree has more than 1 root
                cout << "E4\n";
                return false;
            }
        }
